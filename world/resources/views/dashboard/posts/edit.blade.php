...
@section('body')
    <div class="flex flex-col p-5 space-y-2 bg-white rounded-lg">
        <p>New Article</p>
        <form action="{{ route('post.update', ['id'=>$post->id]) }}" method="POST" class="flex flex-col space-y-2">
            @csrf
            <div class="flex flex-col">
                <label for="title">Title</label>
                <input type="text" name="title" id="title" class="h-10 px-2 py-1 font-bold border" value="{{$post->title}}">
            </div>
            <div class="flex flex-col">
